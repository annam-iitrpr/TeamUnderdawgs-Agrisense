...
| P1-02 | Progressive onboarding and field setup | **integrated** | Six steps built; field creation verified end to end against the live API with a persisted record and proven idempotency. Place search now works against live `/catalog/locations`, so GPS is no longer required to finish. Remaining: soil-card upload, season creation (blocked on the crop catalogue), Playwright coverage |
...
| P1-04 | Home dashboard, field switching, data requests | **in-progress** | Renders live field data with the data-mode badge, honest no-crop/no-recommendation states and blocked-with-reason data requests. Remaining: recommendation card (blocked on Phase 2), seven-day tasks, water and ROI strips, and the two-field race test |
